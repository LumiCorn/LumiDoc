...
LumiDoc 支持 LaTeX 数学公式。例如下面的质能方程：
...
```
此外，LumiDoc 不断尝试适配`行内 LaTeX`来支持在句子中添加数学公式，如质能方程 \$ E=mc^{2} \$ 将会显示在该段落中。（应该是会正常显示的吧？🤔）

除了简单的公式外，你也可以展示一些比较复杂的 LaTeX 公式，比如下面有欧拉、级数、矩阵：
